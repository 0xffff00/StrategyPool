...
namespace StrategyPool
{
    ...
    class PositionApplication
    {
        ...
        private optionPositionChange getChange(optionFormat lastPosition,optionFormat thisPosition)
        {
            ...
            for (int index = 0; index < 5; index++)
            {
                if (askChange.ContainsKey(thisPosition.ask[index].price))
                {
                    ...
                    newStatus.volumn += thisPosition.ask[index].volumn;
                ...
                {
                    optionPriceWithGreek newStatus = new optionPriceWithGreek(thisPosition.ask[index].price, thisPosition.ask[index].volumn, thisPosition.ask[index].volatility, thisPosition.ask[index].delta);
                ...
                if (askChange.ContainsKey(lastPosition.ask[index].price))
                {
                    ...
                    newStatus.volumn -= lastPosition.ask[index].volumn;
                ...
                {
                    optionPriceWithGreek newStatus = new optionPriceWithGreek(lastPosition.ask[index].price, -lastPosition.ask[index].volumn, lastPosition.ask[index].volatility, lastPosition.ask[index].delta);
                    askChange.Add(lastPosition.ask[index].price, newStatus);
                }
            }
            foreach (var item in askChange)
            {
                if (item.Value.volumn!=0)
                {
            ...
            for (int index = 0; index < 5; index++)
            {
                if (bidChange.ContainsKey(thisPosition.bid[index].price))
                {
                    ...
                    newStatus.volumn += thisPosition.bid[index].volumn;
                ...
                {
                    optionPriceWithGreek newStatus = new optionPriceWithGreek(thisPosition.bid[index].price, thisPosition.bid[index].volumn, thisPosition.bid[index].volatility, thisPosition.bid[index].delta);
                ...
                if (bidChange.ContainsKey(lastPosition.bid[index].price))
                {
                    ...
                    newStatus.volumn -= lastPosition.bid[index].volumn;
                ...
                {
                    optionPriceWithGreek newStatus = new optionPriceWithGreek(lastPosition.bid[index].price, -lastPosition.bid[index].volumn, lastPosition.bid[index].volatility, lastPosition.bid[index].delta);
        ...
        public static optionFormat GetPositionShot(optionFormat lastShot,optionPositionChange change)
        {
            ...
            {
                optionPriceWithGreek status =new optionPriceWithGreek(lastShot.ask[i].price, lastShot.ask[i].volumn, lastShot.ask[i].volatility, lastShot.ask[i].delta);
                if (status.price>0 && status.volumn>0)
                {
            ...
            foreach (optionPriceWithGreek status in change.askChange)
            {
                ...
                {
                    ask[status.price] = new optionPriceWithGreek(ask[status.price].price,ask[status.price].volumn+status.volumn,status.volatility,status.delta);
                }
                else
                {
                    ask.Add(status.price, new optionPriceWithGreek(status.price, status.volumn, status.volumn, status.delta));
                }
            }
            int num = 0;
            ...
            foreach (var item in ask)
            {
                if (item.Value.volumn>0)
                {
                    ...
                    nextShot.ask[num].volumn = item.Value.volumn;
            ...
            {
                optionPriceWithGreek status = new optionPriceWithGreek(lastShot.bid[i].price, lastShot.bid[i].volumn, lastShot.bid[i].volatility, lastShot.bid[i].delta);
                if (status.price > 0 && status.volumn > 0)
                {
            ...
            foreach (optionPriceWithGreek status in change.bidChange)
            {
                ...
                {
                    bid[status.price] = new optionPriceWithGreek(bid[status.price].price, bid[status.price].volumn + status.volumn, status.volatility, status.delta);
                }
                else
                {
                    bid.Add(status.price, new optionPriceWithGreek(status.price, status.volumn, status.volumn, status.delta));
                }
            }
            ...
            foreach (var item in bid)
            {
                if (item.Value.volumn > 0)
                {
                    ...
                    nextShot.bid[num].volumn = item.Value.volumn;
